...
export class GateFilter {
  ...
  public isGate(wall: Wall, gridSize: number, fencePoints: Point[]): boolean {
    ...
    const isGateSize = size <= MAX_GATE_TILES + 2 * gridSize && size % gridSize < 2;

    const points: Point[] = [
      { x: x1, y: y1 },
      { x: x2, y: y2 }
    ];
    const sharesFenceCoordinate = points.some(point => this.isPointInFence(point, fencePoints));

    return isGateSize && sharesFenceCoordinate;
  }

  private isPointInFence(point: Point, fencePoints: Point[]): boolean {
    return fencePoints.some(fencePoint => fencePoint.x === point.x && fencePoint.y === point.y);
  }
}
